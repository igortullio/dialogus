...
- [ ] T016 [P] [US1] Integration test: unauthenticated access is 401/redirect, two-user thread isolation, and `listThreads` is `resourceId`-scoped (regression for the all-users leak) in `apps/web/__tests__/integration/thread-isolation.integration.test.ts`.
- [ ] T017 [P] [US1] E2E (Playwright + `@axe-core/playwright`): sign-in → private workspace → sign-out journey with `/` gated; assert Lighthouse a11y ≥ 0.9 and zero axe violations on `/sign-in` in `apps/web/__tests__/e2e/auth-signin.spec.ts`.
...
- [ ] T034 [US2] Add the per-user concurrency cap + membership authorization in `apps/api/src/application/library/{ingest,retryIngest,getIngestionStatus,getChunk}.ts`, and register the `ingestion-concurrency-limit` (429 + `Retry-After`) slug in `problem.ts`.
