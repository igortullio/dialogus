...
- [x] T035 [US2] Implement the preferences application service in `apps/api/src/application/preferences/spoilerCaps.ts` and route `apps/api/src/infrastructure/http/routes/preferences.ts` (GET caps by `book_ids`, PUT one cap; `userId` from session; PUT membership-gated → `book-not-found`; Zod envelope), reusing the `spoiler_caps` shape via `packages/shared/src/schemas/preferences.ts`. Wired into `main()`.
- [x] T036 [US2] Rewrote `apps/web/src/lib/spoiler-cap.ts` to read/write the preferences API via TanStack Query (per-book, account-scoped; `useSpoilerCap(bookId)`, optimistic writes; `readAllSpoilerCaps`/`clearSpoilerCapsForThread` removed) + `apps/web/src/lib/api/preferences.ts` client; updated `ThreadHeader.tsx` (BookChip drops `threadId`), `DialogusThread.tsx` (no client-sent caps), `useThreadCleanup.ts` (no per-thread cap clear).
- [x] T037 [US2] The stream proxy `apps/web/src/app/api/agents/dialogusAgent/stream/route.ts` now sources the authenticated user's caps **server-side** from the preferences API (session cookie forwarded) and overrides any client value — caps are account-scoped and can't be tampered with. (Deeper Mastra `runtimeContext`/tool-default-arg injection against a non-compliant model remains a follow-up; the LLM still re-emits caps into tool args.)
- [x] T038 [US2] `AddGutendexSheet.tsx`: dropped the `duplicate-gutendex-id`/manual-restore workaround (add is idempotent server-side) and the client-side auto-ingest (the API auto-enqueues on add). remove/restore stay user-scoped via the session.
- [x] T039 [US2] FR-022 guard added in `apps/api/__tests__/integration/library.integration.test.ts`: a shared book with no `library_entries` membership never surfaces (empty list incl. `include_deleted`, `book-not-found` by id). Confirmed `BookRepository.list` (global) is orphaned — no HTTP path reaches it.
...
- [x] T040 [P] [US3] Integration test (Testcontainers) in `apps/api/__tests__/integration/admin-invitations.integration.test.ts`: full state machine (pending→used, single-use re-accept rejected), unauthorized account creation rejected + `unauthorized_signup_attempt` audited, `account_created` audited, expired-invite rejected, revoke = ban + session deletion + `access_revoked` audit, and the last-admin guard (revoke + demote). Also unit-tested (services/hooks vs an in-memory `AdminRepository` fake): `__tests__/application/admin/{members,invitations}.test.ts` + `__tests__/infrastructure/auth/hooks.test.ts` (26 unit tests). Built test-first.
- [x] T041 [P] [US3] E2E (Playwright + axe) in `apps/web/__tests__/integration/onboarding.spec.ts` (placed where the `integration` Playwright project discovers it — no separate `e2e` project, per the T028 precedent): invite (admin console) → resolve token → accept → auto sign-in → workspace; `/admin` axe; non-admin redirect; invalid-token error. Typecheck-validated (needs the full stack + seeded owner/member, per T017/T028).

### Implementation for User Story 3

- [x] T042 [US3] Added the `invitations` (partial `UNIQUE(email) WHERE status='pending'`) and `security_events` Drizzle schema in `packages/db/src/schema/{invitations,security_events}.ts` (+ exports); generated migration **`0010_invitations_and_security_events`** (the migration number was `0010`, not `0011` — US2 consolidated into `0009_library_and_preferences`).
- [x] T043 [US3] Added the `databaseHooks.user.create.before` allowlist gate in `apps/api/src/infrastructure/auth/hooks.ts` (`createAllowlistHooks`): normalizes the email, allows an open invitation, **exempts `role:'admin'` creation** (the out-of-band owner/admin bootstrap — `/sign-up/email` stays `disableSignUp:true`, so accounts are created server-side via `internalAdapter.createUser`, which still runs these hooks), else writes `unauthorized_signup_attempt` and throws `APIError('FORBIDDEN')`. The `after` hook consumes the invitation (pending→used). Wired via `createAuth({ adminRepo })`.
- [x] T044 [US3] Event hooks → `security_events`: `account_created` (user.create.after), `sign_in` (session.create.after) in `hooks.ts`; `access_revoked` is written by the admin revoke service (most reliable — it owns the ban+session-delete). `unauthorized_signup_attempt` from the before-hook. (`sign_in_failed` deferred to US4 session-lifecycle work.)
- [x] T045 [US3] Implemented admin application services in `apps/api/src/application/admin/` (`invitations.ts`: create/list/revoke/accept; `members.ts`: list/revoke/restore/role) over an `AdminRepository` port (`ports.ts` + `DrizzleAdminRepository`), with the **last-admin guard** and `errors.ts` (`LastAdminError`, `InvitationInvalidError`, `InvitationConflictError`, `MemberNotFoundError`).
- [x] T046 [US3] Added `apps/api/src/infrastructure/http/routes/admin.ts` behind `requireAdmin` (cursor pagination + Zod envelopes) + the public `routes/invitations.ts` (GET token email, POST accept). Registered `last-admin` (409), `invitation-invalid` (410), `invitation-conflict` (409), `member-not-found` (404) in `problem.ts` (refactored `mapError` into a `DialogusError` sub-mapper to stay ≤15 cognitive complexity). Wired into `main()`.
- [x] T047 [US3] `createInvitation` sends the invite email through the `EmailProvider` port with the accept-invite link `${APP_URL}/accept-invite?invitation={id}` (the invitation id is the unguessable token).
- [x] T048 [US3] Built `apps/web/src/app/(auth)/accept-invite/page.tsx` (+ `AcceptInviteForm`) and the admin console `apps/web/src/app/admin/page.tsx` (server-gated to admins) with `components/admin/{InvitationsPanel,MembersPanel,AdminDashboard}` (TanStack Query, invite/revoke/role) + admin link in `AccountMenu`; `lib/api/{admin,invitations}.ts` clients. shadcn/ui + Tailwind tokens, keyboard-accessible.
...
- [x] T055 [P] Account deletion (FR-023): `deleteAccount` service (last-admin guard → delete Mastra threads by `resourceId` via the `UserThreadDeleter` port / `MastraThreadDeleter` → delete the user row, whose FKs cascade `session`/`account`/`library_entries`/`user_book_preferences` and SET NULL `security_events`/`invitations`). Exposed as `DELETE /api/admin/members/:id` + an "Excluir" affordance (confirm dialog) in the admin members panel. Testcontainers cascade test (`apps/api/__tests__/integration/account-deletion.integration.test.ts`) asserts A removed, B + shared corpus untouched, audit anonymized, threads deleter called, last-admin refused. (Self-delete endpoint deferred — the admin path satisfies FR-023.)
- [x] T056 [P] ADRs written under [`specs/001-multi-user-auth/adrs/`](./adrs/) (co-located with the rest of the feature's design docs; the `.compozy/tasks/` path was the old convention): `adr-001-auth-library` (Better Auth), `adr-002-auth-error-exemption` (E1), `adr-003-mastra-table-ownership` (E2).
- [x] T057 [P] `README.md` gained a "Multi-user auth & onboarding (feature 001)" section (setup, owner seed, mock email mode, invite/accept/reset flows, single-origin deployment, auth/admin error slugs). `.env.example` already carried the auth/email vars from Foundational T004.
- [x] T058 Finalized cookie/CORS: documented the single-origin (`SameSite=Lax; Secure; HttpOnly`) model vs the cross-origin fallback (`SameSite=None; Secure` + explicit-origin credentialed CORS) in `auth.ts`; added FR-021 reset rate-limit `customRules` (`/request-password-reset` 5/h, `/reset-password` 10/h). (The `/api/invitations/accept` Hono-level rate limit remains a noted follow-up — it's a uuid-token lookup, low risk.)
- [x] T059 [P] Closed the real gaps: **moved** the email-provider-selection test from `src/` into `__tests__/` (it was orphaned and never collected by the gate) and added env-validation unit tests for the new auth/email vars (defaults, numeric coercion + positivity, `EMAIL_PROVIDER` enum, `APP_URL` format) in `packages/shared/__tests__/config.test.ts`. The invitation state machine, last-admin guard, and idempotent membership add were already TDD-covered in US2/US3.
- [x] T061 Pre-commit gate (`pnpm lint && pnpm -r typecheck && pnpm -r test`) is green — 1553 unit tests across all packages; ran on every commit of this feature.
- [~] T060 Per-page **axe** coverage is in the E2E specs (`/sign-in` in auth-gate, `/library` in library-isolation, `/admin` in onboarding, `/reset-password` in password-reset) — typecheck-validated. The full `quickstart.md` 1–8 walkthrough + Lighthouse a11y ≥ 0.9 require the **live stack + seeded users** and are executed in CI / manual validation, not in the unit gate.
- [~] T062 [P] SC-006 concurrency-isolation E2E written (`apps/web/__tests__/integration/concurrency-isolation.spec.ts`, placed where the `integration` Playwright project discovers it): N concurrent users (env-parameterized via `E2E_CONCURRENT_USERS`, ≥10 in CI) each reach their own workspace; thread lists are fetched concurrently and asserted to have zero cross-user overlap. Typecheck-validated; executes in CI with the live stack + `E2E_MOCK_LLM=1` (the streaming-latency "no perceptible degradation" aspect is a documented qualitative check).
